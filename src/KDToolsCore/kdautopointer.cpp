...
/*!
  ...
  For the same reason, the \ref KDAUTOPOINTER_NO_IMPLICIT_CONVERSION macro
  can be defined so that if you need to get a pointer to the contained
  (owned) object, the conversion is not implicit, either. You have to
  explicitly call get() (if you want to leave ownership to the
  KDAutoPointer) or release() (if you want to transfer ownership out
  of KDAutoPointer).

  For convenience, however, \ref KDAUTOPOINTER_NO_IMPLICIT_CONVERSION is
  ...
  KDAutoPointers, you do not have to care about whether the object
  deletes itself or not. If it deletes itself, then KDAutoPointer will set itself
  ...
  QDialog::exec() returns QDialog::Rejected when the dialog has been
  deleted, so we use the same code for the rejected and the deleted
  case, and simply return without touching \c dialog again.
*/
...
/*!
  \fn KDAutoPointer::operator T *() const
  \since_f 2.3
  ...
  \note This operator is only available if
  \ref KDAUTOPOINTER_NO_IMPLICIT_CONVERSION is \em not defined.
*/
...
/*!
  ...
  Define this macro to disable the implicit conversion from
  KDAutoPointer<T> to T*.

  This conversion is convenient for passing the autopointer to
  QObject::connect() or as a parent to QObject constructors, however it
  can be disabled to conform more to std::auto_ptr.
*/
# define KDAUTOPOINTER_NO_IMPLICIT_CONVERSION
#endif
...
KDAB_UNITTEST_SIMPLE( KDAutoPointer, "kdtools/core" ) {
